...
const teaserPath = path.join(projectRoot, "social-media", "video", "scoregame-teaser-trail.html");
const outputDir = path.join(projectRoot, "social-media", "teasers");
const outputPath = path.join(outputDir, "scoregame-trail-teaser.webm");
const coverPath = path.join(outputDir, "scoregame-trail-teaser-cover.png");
const actionPath = path.join(outputDir, "scoregame-trail-teaser-action.png");

test("exports the ScoreGame trail teaser", async ({ page }) => {
  const pageErrors: string[] = [];
  page.on("pageerror", (error) => pageErrors.push(error.stack || String(error)));

  await page.route("http://scoregame.local/social-media/video/scoregame-teaser-trail.html", async (route) => {
    await route.fulfill({
      status: 200,
  ...
  await page.goto("http://scoregame.local/social-media/video/scoregame-teaser-trail.html");
  await page.waitForFunction(() => window.__teaserReady === true);

  await page.evaluate(() => window.renderTeaserFrame(3200));
  ...
  expect(download.suggestedFilename()).toBe("scoregame-teaser-trail.webm");
  expect(statSync(outputPath).size).toBeGreaterThan(100_000);

  await page.route("http://scoregame.local/scoregame-trail-teaser.webm", async (route) => {
    await route.fulfill({
      status: 200,
  ...
  const metadata = await page.locator("#result").evaluate(async (video: HTMLVideoElement) => {
    const response = await fetch("http://scoregame.local/scoregame-trail-teaser.webm");
